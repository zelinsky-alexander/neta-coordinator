...
public final class CertificateIssuer {
    ...
    public CertificateIssuer(CoordinatorProperties properties) {
        this.properties = properties;
    }

    public IssuedCertificate issue(String agentId, String csrPem) {
        if (csrPem == null || csrPem.isBlank()) {
            throw ProtocolException.badRequest("csr_pem is required");
        }
        ...
        try {
            ...
            var builder = new JcaX509v3CertificateBuilder(
                    ...
                    Date.from(now.plus(config.certificateTtl())),
                    subject,
                    publicKey);
            ...
            throw new IllegalStateException("failed to issue agent certificate", e);
        }
    }
    ...
    private static IssuerMaterial loadIssuer(CoordinatorProperties.Enrollment config) throws Exception {
        ...
        try (var input = Files.newInputStream(Path.of(config.issuerKeyStore()))) {
            keyStore.load(input, password);
        }
        String alias = config.issuerKeyAlias();
        if (!keyStore.isKeyEntry(alias)) {
            throw new IllegalStateException("issuer key alias not found: " + alias);
        }
        PrivateKey privateKey = (PrivateKey) keyStore.getKey(alias, password);
        X509Certificate certificate = (X509Certificate) keyStore.getCertificate(alias);
        if (privateKey == null || certificate == null) {
            throw new IllegalStateException("issuer key store does not contain a private key and certificate");
        }
        if (certificate.getBasicConstraints() < 0) {
            throw new IllegalStateException("configured enrollment issuer certificate is not a CA certificate");
        }
