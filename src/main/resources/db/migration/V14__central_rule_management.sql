...
INSERT INTO rule_definitions(rule_id,revision,origin,name,category,severity,enabled,parameters_json)
VALUES
('NETA-PERF-001',1,'DEFAULT','Network path degradation','performance','medium',true,
...
('NETA-TRUST-001',1,'DEFAULT','Outbound TLS identity','trust','high',true,
 '{"require_chain_valid":true,"require_hostname_valid":true,"compare_spki":true}'::jsonb),
('NETA-TRUST-002',1,'DEFAULT','Inbound authenticated TLS identity','trust','high',true,
...
('NETA-PROC-001',1,'DEFAULT','Execution from transient path','process','medium',true,
 '{"path_prefixes":["/tmp/","/var/tmp/","/dev/shm/"],"path_substrings":["/appdata/local/temp/","/windows/temp/"]}'::jsonb),
('NETA-PROC-002',1,'DEFAULT','Shell from unexpected parent','process','medium',true,
...
('NETA-PROC-003',1,'DEFAULT','Unexpected elevation','process','medium',true,
 '{"expected_parent_names":["sudo","su","pkexec","doas","consent.exe"]}'::jsonb),
('NETA-PROC-004',1,'DEFAULT','Rapid child process fanout','process','medium',true,
 '{"child_count":6,"window_ms":10000}'::jsonb),
('NETA-PROC-005',1,'DEFAULT','Short-lived process burst','process','medium',true,
